...
- [x] Preview reviewed import details locally in Recipe Browser without routing through the curated verified Recipe Detail page.
- [x] Edit reviewed-import cleanup fields for title, ingredients, and instructions while preserving provenance and reviewed-import status.
- [x] Define explicit promotion guardrails before any reviewed import can become a curated verified recipe.
- [x] Define future promotion workflow stages, audit checklist, API shape, UI shape, copy rules, tests, and non-goals.
- [x] Surface a read-only promotion readiness audit in the reviewed-import preview without adding a promotion action.
- [x] Persist promotion audit checklist state and reviewer notes without promoting reviewed imports.

Phase 11A creates the foundation, Phase 11B persists the queue, Phase 11C adds the review UI, and Phase 11D adds a controlled backend import path. Phase 12 exposes the reviewed imported layer in Recipe Browser with trust labels. Phase 13 lets reviewed imports participate in pantry-aware ranking in their own lane while preserving source/provenance labels. Phase 14 adds a reviewed-import detail preview that keeps provenance/trust labels visible, shows preserved ingredients and instructions, and avoids the curated verified Recipe Detail page. Phase 15 adds reviewed-import cleanup/editing for title, ingredients, and instructions. Cleanup preserves provenance/trust labels, does not use the curated verified Recipe Detail page, and does not promote imports into curated verified recipes. Phase 16 defines promotion guardrails in `p2p-reviewed-import-promotion-guardrails.md`; promotion still requires a future explicit workflow. Phase 17 defines that workflow design and audit checklist in `p2p-reviewed-import-promotion-workflow.md`; promotion still is not implemented. Phase 18 surfaces a read-only promotion readiness audit in the reviewed-import preview while preserving separation and avoiding any curated verified write. Phase 19 persists promotion audit checklist state and reviewer notes for reviewed imports without adding a promotion action or curated verified write. Imported records remain separate from the curated verified recipe bank until that workflow exists and succeeds.
