...
Phase 11A creates the foundation, Phase 11B persists the queue, Phase 11C adds the review UI, and Phase 11D adds a controlled backend import path. Phase 12 exposes the reviewed imported layer in Recipe Browser with trust labels. Phase 13 lets reviewed imports participate in pantry-aware ranking in their own lane while preserving source/provenance labels. Phase 14 adds a reviewed-import detail preview that keeps provenance/trust labels visible, shows preserved ingredients and instructions, and avoids the curated verified Recipe Detail page. Phase 15 adds reviewed-import cleanup/editing for title, ingredients, and instructions. Cleanup preserves provenance/trust labels, does not use the curated verified Recipe Detail page, and does not promote imports into curated verified recipes. Phase 16 defines promotion guardrails in `p2p-reviewed-import-promotion-guardrails.md`; promotion still requires a future explicit workflow. Imported records remain separate from the curated verified recipe bank until that workflow exists and succeeds.
