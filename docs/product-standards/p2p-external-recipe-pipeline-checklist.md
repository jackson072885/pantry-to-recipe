...
Phase 11A creates the foundation, Phase 11B persists the queue, Phase 11C adds the review UI, and Phase 11D adds a controlled backend import path. Phase 12 exposes the reviewed imported layer in Recipe Browser with trust labels. Phase 13 lets reviewed imports participate in pantry-aware ranking in their own lane while preserving source/provenance labels. Phase 14 adds a reviewed-import detail preview that keeps provenance/trust labels visible, shows preserved ingredients and instructions, and avoids the curated verified Recipe Detail page. Imported records remain separate from the curated verified recipe bank until a future promotion process exists. A future phase may add imported recipe editing or cleanup before any promotion workflow.
