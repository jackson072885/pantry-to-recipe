...
Phase 11A adds an import-review foundation for external candidates. Phase 11B persists that review queue for create/read/list/update workflows. Phase 11D adds a controlled backend import path into a separate imported recipe layer. The queue and import path preserve source provenance, normalized display fields, readiness metadata, reviewer notes, and deterministic safety flags, but imported external recipes are still not curated verified recipes.
