...
The save-winner / recipe import flow now has review queue persistence, review UI, a controlled backend approved-import path, visible reviewed-import surfacing with trust labels, a separate pantry-fit ranking lane, a local reviewed-import detail preview, reviewed-import cleanup/editing for title, ingredients, and instructions, explicit promotion guardrails, and a promotion workflow/audit design. Cleanup preserves provenance, stays out of the curated verified Recipe Detail page, and does not promote imports into curated verified recipes. A future phase may implement an explicit promotion workflow only after the provenance audit, quality review, duplicate check, and safety gates defined in `p2p-reviewed-import-promotion-guardrails.md` and `p2p-reviewed-import-promotion-workflow.md`.
