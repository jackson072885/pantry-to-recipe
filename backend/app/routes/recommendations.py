...
def list_recommendations(
    ...
    return route_response(
        lambda: _build_recommendations(db, pantry),
        db=db,
...
def _build_recommendations(db: Session, pantry: list[str]) -> dict:
    if not pantry:
    ...
    result = recommend_recipes(db, pantry)
    logger.info(
        "Recommendation response: pantry_items=%s cook_now=%s almost_there=%s not_worth_it=%s",
        len(pantry),
        len(result["cook_now"]),
